...
import { scrapeFacebookProfile, scrapeFacebookReels } from "@/lib/scraper"
...
async function insertDailySnapshot(
  ...
  if (error) {
    console.error("[facebook-sync] video_daily_stats upsert error:", error.message)
  }
}
...
async function insertFollowerSnapshot(
  ...
  if (error) {
    console.error("[facebook-sync] follower_snapshots upsert error:", error.message)
  }
}

export async function syncFacebookFromScraper(
  supabase: SupabaseClient,
