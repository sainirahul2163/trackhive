...
interface CompAccount {
  id: string; competitor_id: string; platform: Platform; username: string
  avatar_url: string | null; follower_count: number; total_views: number
  avg_views: number; posting_frequency: number
}
interface Competitor {
  id: string; name: string; website: string; logo_url: string | null
  accounts: CompAccount[]
}
interface CompVideo {
  id: string; competitor_account_id: string; platform: Platform
  thumbnail_url: string | null; caption: string; views: number; likes: number
  engagement_rate: number; content_format: string; posted_at: string
}
interface CompCreator {
  id: string; competitor_id: string; creator_handle: string; platform: Platform
  avatar_url: string | null; follower_count: number; avg_views: number
  videos_posted: number; status: "active" | "inactive"; flagged_outreach: boolean
  last_seen_at: string
}
interface AiReport {
  id: string; competitor_id: string; week_of: string; summary: string
  top_videos: { url: string; views: number; caption: string }[]
...
const MOCK_COMPETITORS: Competitor[] = [
  {
    id: "comp1", name: "GrowthBrands Co",  website: "growthbrands.co",  logo_url: null,
    accounts: [
      { id: "ca1", competitor_id: "comp1", platform: "tiktok",    username: "@growthbrands_tt", avatar_url: null, follower_count: 890000,  total_views: 42000000, avg_views: 680000, posting_frequency: 4.2 },
      { id: "ca2", competitor_id: "comp1", platform: "instagram", username: "@growthbrands",    avatar_url: null, follower_count: 640000,  total_views: 18000000, avg_views: 290000, posting_frequency: 2.8 },
    ],
  },
  {
    id: "comp2", name: "ViralPush Inc",    website: "viralpush.com",    logo_url: null,
    accounts: [
      { id: "ca3", competitor_id: "comp2", platform: "tiktok",    username: "@viralpush",       avatar_url: null, follower_count: 1200000, total_views: 78000000, avg_views: 920000, posting_frequency: 6.1 },
      { id: "ca4", competitor_id: "comp2", platform: "youtube",   username: "@viralpush_yt",    avatar_url: null, follower_count: 380000,  total_views: 24000000, avg_views: 410000, posting_frequency: 1.5 },
    ],
  },
  {
    id: "comp3", name: "NexGen Creator",   website: "nexgencreator.io", logo_url: null,
    accounts: [
      { id: "ca5", competitor_id: "comp3", platform: "instagram", username: "@nexgencreator",   avatar_url: null, follower_count: 520000,  total_views: 14000000, avg_views: 220000, posting_frequency: 3.4 },
      { id: "ca6", competitor_id: "comp3", platform: "tiktok",    username: "@nexgen_tt",       avatar_url: null, follower_count: 310000,  total_views: 9000000,  avg_views: 180000, posting_frequency: 2.9 },
    ],
  },
]

const THUMBS = ["comp1","comp2","comp3","comp4","comp5","comp6","comp7","comp8"]
  .map(s => `https://picsum.photos/seed/${s}/200/120`)

function buildVideos(competitorAccountId: string, platform: Platform): CompVideo[] {
  return Array.from({ length: 6 }, (_, i) => ({
    ...
    content_format: ["hook_first","testimonial","before_after","lifestyle","storytime","product_demo"][i],
    posted_at:      new Date(Date.now() - i * 86400000 * 3).toISOString(),
  }))
}

function buildCreators(competitorId: string): CompCreator[] {
  return [
    { id: `${competitorId}-cr1`, competitor_id: competitorId, creator_handle: "@mikecreates",  platform: "tiktok",    avatar_url: "https://api.dicebear.com/7.x/avataaars/svg?seed=mike",  follower_count: 890000,  avg_views: 680000, videos_posted: 8,  status: "active",   flagged_outreach: false, last_seen_at: new Date(Date.now()-86400000).toISOString()    },
    { id: `${competitorId}-cr2`, competitor_id: competitorId, creator_handle: "@sarahlifts",   platform: "instagram", avatar_url: "https://api.dicebear.com/7.x/avataaars/svg?seed=sarah", follower_count: 540000,  avg_views: 310000, videos_posted: 5,  status: "active",   flagged_outreach: true,  last_seen_at: new Date(Date.now()-172800000).toISOString()   },
    { id: `${competitorId}-cr3`, competitor_id: competitorId, creator_handle: "@techwithtom",  platform: "youtube",   avatar_url: "https://api.dicebear.com/7.x/avataaars/svg?seed=tom",  follower_count: 280000,  avg_views: 190000, videos_posted: 3,  status: "inactive", flagged_outreach: false, last_seen_at: new Date(Date.now()-2592000000).toISOString()  },
    { id: `${competitorId}-cr4`, competitor_id: competitorId, creator_handle: "@fitnessfiona", platform: "tiktok",    avatar_url: "https://api.dicebear.com/7.x/avataaars/svg?seed=fiona",follower_count: 1100000, avg_views: 940000, videos_posted: 12, status: "active",   flagged_outreach: false, last_seen_at: new Date(Date.now()-3600000).toISOString()     },
  ]
}
...
function buildReports(competitorId: string): AiReport[] {
  return [
    {
      id: `${competitorId}-r1`, competitor_id: competitorId,
    ...
    {
      id: `${competitorId}-r2`, competitor_id: competitorId,
...
export default function DemoCompetitorsPage() {
  ...
  const [creators, setCreators] = useState<Record<string, CompCreator[]>>({})
  const [videos, setVideos] = useState<Record<string, CompVideo[]>>({})
  ...
  useEffect(() => {
    ...
    if (!videos[selectedId]) {
      ...
      const allVids = (comp?.accounts ?? []).flatMap(acc => buildVideos(acc.id, acc.platform))
      setVideos(p => ({ ...p, [selectedId]: allVids }))
    }
  }, [selectedId]) // eslint-disable-line react-hooks/exhaustive-deps

  function handleSelect(id: string) { setSelectedId(id); setActiveTab("Overview") }

  function toggleFlag(creatorId: string) {
    if (!selectedId) return
    openGate("creator outreach tracking")
    void creatorId
  }

  function handleGenerateReport() {
    openGate("AI competitor reports")
  }
  ...
  return (
    <div style={{ display: "flex", gap: "0", height: "calc(100vh - 148px)", overflow: "hidden" }}>

      {/* ── LEFT SIDEBAR ─────────────────────────────────────── */}
      <div style={{ width: "280px", flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.06)", overflowY: "auto", backgroundColor: "#0a0a0a" }}>
        <div className="flex items-center justify-between px-4 py-4 border-b border-white/[0.06] flex-shrink-0">
          ...
          <button
            onClick={() => openGate("adding competitors")}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected ? "bg-purple-600/10 border border-purple-500/30" : "hover:bg-white/[0.03] border border-transparent"
                )}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                  ...
                  <div className="xl:col-span-2 rounded-xl border border-white/[0.06] bg-[#111111] p-5">
                    ...
                    <div className="flex items-center gap-4 mt-3">
                      ...
                      <div className="flex items-center gap-1.5"><div className="w-3 h-0.5 bg-emerald-500 rounded" /><span className="text-[11px] text-zinc-500">Your workspace</span></div>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
                  ...
                  <div className="flex gap-2">
                    {(["tiktok","instagram","youtube","facebook"] as Platform[])
                      .filter(p => selected.accounts?.some(a => a.platform === p))
                      .map(p => {
                        const cfg = PLATFORM_CONFIG[p]
                        return (
                          <span key={p} style={{ backgroundColor: cfg.bgColor, color: cfg.fgColor, padding: "4px 8px", borderRadius: "6px", fontSize: "11px", fontWeight: 500 }}>
                            {cfg.label}
                          </span>
                        )
                      })}
                      ...
                      return (
                        <tr key={v.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-4 py-3.5 text-right">
                            <span className={`text-sm font-medium ${v.engagement_rate > 8 ? "text-emerald-400" : "text-zinc-400"}`}>{v.engagement_rate}%</span>
                          </td>
                            ...
                            <div className="flex items-center gap-2.5">
                              <Avatar className="w-8 h-8">
                                ...
                                <AvatarFallback className="bg-purple-600/20 text-purple-400 text-xs font-bold">
                                  {c.creator_handle.slice(1, 3).toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                                <div className="flex items-center gap-1.5">
